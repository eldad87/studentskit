...
(function ($) {
    ...
    function drawTip(canvas, coordinates, color) {
        ...
        var context = canvas.get(0).getContext('2d');
        context.fillStyle = color;
        context.fillStyle   = this.options.style.background; // blue
        context.strokeStyle = '#cccccc'; // red
        // Create tip
        context.beginPath();
    ...
    function createTip(corner) {
        ...
        // Create tip element
        self.elements.tip = '<div class="topimp ' + self.options.style.classes.tip + '" dir="ltr" rel="' + corner.string() + '" style="position:absolute; ' + 'height:' + self.options.style.tip.size.height + 'px; width:' + self.options.style.tip.size.width + 'px; ' + 'margin:0 auto; line-height:0.1px; font-size:1px;z-index:15050;"></div>';

        // Attach new tip to tooltip element
        self.elements.tooltip.prepend(self.elements.tip);

        // Use canvas element if supported
        if($('<canvas />').get(0).getContext) { tip = '<canvas tail="yes" height="' + self.options.style.tip.size.height + '" width="' + self.options.style.tip.size.width + '"></canvas>'; }
