	...
	function Calendar() {
		...
		*/
		...
		this.do_init = function(){
			...
			for(var dayIndex = 0; dayIndex < Calendar.dayNames.length; dayIndex++){
				...
				if(dayIndex < firstDayWkIndex){
					...
					calWeekHeaderCellObj.addClass("JFrontierCal-PrevMonth-Day-Cell");
				...
				this.dayHash.put(
					...
					calDayCell);
				if((!showTodayStyle || dayNum<=currentDayNum )&& cM>=currentMonthNum ){
					calWeekHeaderCellObj.addClass("JFrontierCal-Day-Cell-Today-prv");
					calDayCell.addClass("JFrontierCal-Day-Cell-Today-prv");
				}
			...
			for(var dayIndex = 0; dayIndex < Calendar.dayNames.length; dayIndex++){
				...
				if(dayNum <= daysInCurrentMonth){
					...
					calWeekHeaderCellObj.addClass("JFrontierCal-NextMonth-Day-Cell");
		...
		this.renderAgendaDivElement = function(agi,displayMessage,startDayObject,endDayObject,leftEnd,rightEnd){
			...
			if(nextY > 0){
				...
				if(this.dragAndDropEnabled){
					...
					//d.draggable("enable"); //Not completable with jQuery ui 1.9.1
		...
		this.resize = function(){
			...
			//changed by Raj
			var borderSize = (firstDayCell.jqyObj.outerWidth(true) - firstDayCell.jqyObj.width()) * Calendar.dayNames.length;
			var cellWidth = (Math.floor(calWidth / Calendar.dayNames.length) - (firstDayCell.jqyObj.outerWidth(true) - firstDayCell.jqyObj.width())+3);
			var cellWidthLast = cellWidth + ( calWidth - (cellWidth * Calendar.dayNames.length)) - (firstDayCell.jqyObj.outerWidth(true) - firstDayCell.jqyObj.width())+2;
