...
class Category extends AppModel {
    ...
    private function _add($category, $locale='eng', $id=null, $parentId=null) {
        ...
        if($parentId) {
            $save['parent_category_id'] = $parentId;
        }
        return $this->save( $save );
    }

    public function getAllCategoriesOptions() {
        $categories = $this->find('threaded', array('parent'=>'parent_category_id'));
        $return = array();
    ...
    private function _getAllThreaded(&$return, $categories, $deep=0) {
        ...
        foreach($categories AS $category) {
            $return[$category['Category']['category_id']] = $deepStr.$category['Category']['name'];
    ...
    public function afterSave($created) {
        ...
        $event = new CakeEvent('Model.Category.afterSave', $this, array('category_id'=>$this->id, 'created'=>$created) );
