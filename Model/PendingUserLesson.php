...
class PendingUserLesson extends AppModel {
    ...
    public function execute($pendingUserLessonId) {
        ...
        switch($pendingUserLessonData['action']) {
            ...
            case 'negotiate':
                ...
                $res = $ulObj->reProposeRequest($pendingUserLessonData['user_lesson_id'], $pendingUserLessonData['student_user_id'], $pendingUserLessonData['extra'], $pendingUserLessonData['version']);
                break;

            case 'accept':
                $res = $ulObj->acceptRequest($pendingUserLessonData['user_lesson_id'], $pendingUserLessonData['student_user_id'], $pendingUserLessonData['version']);
                break;
        }
    ...
    public function getUnusedUserLessonId() {
        ...
        $ulObj->create(false);
        $dummyData = array('subject_id'=>0, 'student_user_id'=>0, 'stage'=>0, 'name'=>0, 'description'=>0, 'language'=>0, 'duration_minutes'=>0, 'price'=>0);
